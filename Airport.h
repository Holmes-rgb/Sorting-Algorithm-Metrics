...
private:
    int OBJECTID;
    std::string the_geom, Fac_Type, Field_Office, State_Name, County, City, Fac_Name, Fac_Use, Sectional, Fuel_Types;
...
public:
    ...
    // Getters
    std::string getTheGeom() const {
        return the_geom;
    }

    std::string getFacType() const {
        return Fac_Type;
    }

    std::string getFieldOffice() const {
        return Field_Office;
    }
    ...
    std::string getFacName() const {
        return Fac_Name;
    }

    std::string getFacUse() const {
        return Fac_Use;
    }

    std::string getSectional() const {
        return Sectional;
    }

    std::string getFuelTypes() const {
        return Fuel_Types;
    }
    ...
    bool isAtcTower() const {
        return Atc_Tower;
    }

    bool isLandingFee() const {
        return Landing_Fee;
    }

    bool isWindIndic() const {
        return Wind_Indic;
    }
    ...
    void setAtcTower(bool atcTower) {
        this->Atc_Tower = atcTower;
    }

    void setLandingFee(bool LandingFee) {
        this->Landing_Fee = LandingFee;
    }

    void setWindIndic(bool windIndic) {
        this->Wind_Indic = windIndic;
    }
    ...
    friend std::ostream& operator << (std::ostream& outs, const Airport& kObj) {
        outs << std::right                      // right align numbers
             ...
             << "  "
             << std::setw(50) << kObj.getTheGeom()
             << std::setw(15) << kObj.getFacType()
             << std::setw(16) << kObj.getFieldOffice()
             << std::setw(18) << kObj.getStateName()
             << std::setw(23) << kObj.getCounty()
             << std::setw(27) << kObj.getCity()
             << std::setw(46) << kObj.getFacName()
             << std::setw(16) << kObj.getFacUse()
             << std::setw(12) << kObj.getElevation()
             << std::setw(20) << kObj.getSectional()
             << std::setw(19) << kObj.getFuelTypes()
             << std::setw(11) << kObj.isAtcTower()
             << std::setw(10) << kObj.getUnicom()
             << std::setw(10) << kObj.getCtaf()
             << std::setw(14) << kObj.isLandingFee()
             << std::setw(13) << kObj.isWindIndic();
...
bool loadFromFile(std::string fn, std::vector<Airport>& objs) {
    ...
    while (fin && fin.peek() != EOF) {
        ...
        Airport airport(OBJECTID, the_geom, Fac_Type, Field_Office, State_Name, County, City, Fac_Name,
                        Fac_Use, Sectional, Fuel_Types, Elevation, Unicom, Ctaf, Atc_Tower, Landing_Fee, Wind_Indic);
...
void verifyAllFieldsSet(std::vector<Airport>& objs) {
    ...
    for (Airport& airport : objs) {
        ...
        if (FacName.compare("NONE") == 0) {
            std::cout << "Bad value for Airport name at record "
                      << i << std::endl;
        }
        i += 1;
    }
...
bool AirportTest() {
    ...
    Airport testObject2 = Airport(1, "Fred", "PU", "LAX", "Vermont",
                                  "Chittenden", "Burlington", "JFK","PU",
                                  "MONTREAL", "Jet-A", 0.0, 123.45, 678.90, true, true, true);
    if (testObject2.getObjectid() != 1 ||
        testObject2.getTheGeom() != "Fred" ||
        ...
        testObject2.getFacName() != "JFK" ||
        testObject2.getFacUse() != "PU" ||
        testObject2.getElevation() != 0.0 ||
        testObject2.getSectional() != "MONTREAL" ||
        testObject2.getFuelTypes() != "Jet-A" ||
        !testObject2.isAtcTower() ||
        testObject2.getUnicom() != 123.45 ||
        testObject2.getCtaf() != 678.90 ||
        !testObject2.isLandingFee() ||
        !testObject2.isWindIndic()){
